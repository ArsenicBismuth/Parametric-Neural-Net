  ...
  always @(posedge clk or posedge rst) begin
    if (rst) begin
      ...
      // Load data    
        ...
        if (nd_dout == {{n-12{1'b0}}, 12'h888}) begin
          we = we >> 1;  // Next layer if reading 888 (non-frac)
          $display("[Next Node]");
        end else if (nd_dout == {{n-12{1'b0}}, 12'h999}) begin   // Stop
          we = {nd{1'b0}};
          state = mode;
      ...
      // Backpropagation  
        ...
        for (epoch=0; epoch<max_epoch; epoch=epoch+1) begin
          $display("[Epoch-%02d]", epoch);
          if (data < batch) begin
          
            $display("[Data-%02d]", data);  
            y_we = {8{1'b1}};  // Write last output
